...
def walk_path(network, routes, current_route, small_nodes_walked):
	for connection_index in current_route[-1].connections:
		...
		if connection.name != "end" and connection.name != "start" and connection.name not in small_nodes_walked:
			new_small_nodes_walked = small_nodes_walked.copy()
			if connection.name.islower():
				new_small_nodes_walked.append(connection.name)
			walk_path(network, routes, new_route, new_small_nodes_walked)
...
def main():
	...
	routes = []
	walk_path(network, routes, [network['start']], [])
	print(len(routes))
